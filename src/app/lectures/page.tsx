  ...
  return (
    <div className="py-12 space-y-32">
      ...
      {isEmpty ? (
        ...
        <>
          {books.length > 0 && <section className="space-y-12"><FadeIn direction="right" className="flex items-center gap-6"><h2 className="text-3xl md:text-4xl font-serif text-bb-rose shrink-0 italic">Ce mois-ci</h2><div className="h-[1px] w-full bg-gradient-to-r from-bb-beige to-transparent"></div></FadeIn><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{books.map((book: any, index: number) => <div key={book.id}><p className="mb-2 text-xs font-bold uppercase tracking-widest text-bb-rose">{book.city}</p><BookCard title={book.title} author={book.author} coverUrl={book.coverUrl} month={book.month} resume={book.resume} lien_place_des_libraires={book.lien_place_des_libraires} delay={index * 0.1} /></div>)}</div></section>}
