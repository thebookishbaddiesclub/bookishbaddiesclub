...
export default function AdminPage() {
  ...
  const [bookArchived, setBookArchived] = useState(false);
  ...
  const resetBookForm = () => {
    setEditingId(null);
    setBookTitle(""); setBookAuthor(""); setBookMonth(""); setBookCity("Perpignan"); setBookImage(null); setCurrentBookCover(""); setBookResume(""); setBookLink(""); setBookArchived(false); setBookRating(""); setBookReview("");
  };
    ...
    try {
      ...
      const res = await fetch("/api/admin", {
        ...
        body: JSON.stringify({
          type: "lectures",
          id: editingId,
          data: { title: bookTitle, author: bookAuthor, month: bookMonth, city: bookCity, coverUrl: imageUrl, resume: bookResume, lien_place_des_libraires: bookLink, archived: bookArchived, rating: bookRating ? Number(bookRating) : null, review: bookReview || null }
        })
      });
  ...
  const startEditBook = (book: any) => {
    ...
    setBookArchived(Boolean(book.archived)); setBookRating(book.rating?.toString() || ""); setBookReview(book.review || "");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pt-10">
        ...
        <FadeIn delay={0.2} className="space-y-8 bg-white/40 backdrop-blur-sm p-10 rounded-[3rem] border border-bb-beige shadow-sm">
          ...
          <form onSubmit={handleAddOrEditBook} className="space-y-4">
            ...
            <label className="flex items-center gap-3 px-2 text-sm text-bb-ink/70"><input type="checkbox" checked={bookArchived} onChange={e => setBookArchived(e.target.checked)} /> Lecture archivée</label>
            <input type="number" min="0" max="5" step="0.5" placeholder="Note du club sur 5 (si archivée)" className="w-full bg-white px-8 py-5 rounded-3xl border border-bb-beige" value={bookRating} onChange={e => setBookRating(e.target.value)} />
            {bookArchived && <textarea placeholder="Avis du club" className="w-full bg-white px-8 py-5 rounded-3xl border border-bb-beige min-h-[100px]" value={bookReview} onChange={e => setBookReview(e.target.value)} />}
